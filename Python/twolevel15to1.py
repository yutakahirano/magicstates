...
# Calculates the output error and cost of the (15-to-1) protocol in conjunction
# with the zero-level distillation with trivial embedding with a physical error
# rate pphys, the output error rate and the failure rate of the zero-level
# distillation, distances dx2, dz2 and dm2.
def cost_of_zero_plus_15to1_with_trivial_embedding(pphys, pout0, pfail0, dx2, dz2, dm2):
    # Introduce shorthand notation for logical error rate with distances dx2/dz2/dm2
    px2 = plog(pphys, dx2)
    pz2 = plog(pphys, dz2)
    pm2 = plog(pphys, dm2)
    ...
    # nqubits = 2 * ((dx2 + 4 * dz2) * 3 * dx2 + nl1 * (
    #             (dx + 4 * dz) * (3 * dx + dm2 / 2) + 2 * dm) + 20 * dm2 * dm2 + 2 * dx2 * dm2)
    nqubits = 2 * ((dx2 + 4 * dz2) * 3 * dx2 + 16 * dm2 * dm2)

    ncycles = 7.5 * l1time / (1 - float(pfail2))
    print('zero-plus-one-trivial with pphys=', pphys, ', dx2=', dx2, ', dz2=', dz2,
          ', dm2=', dm2, sep='')
    print('Output error: ', '%.4g' % pout, sep='')
    print('Failure probability: ', '%.3g' % pfail2, sep='')
    print('Qubits: ', '%.0f' % nqubits, sep='')
