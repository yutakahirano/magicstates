...
def cost_of_two_level_20to4(pphys, dx, dz, dm, dx2, dz2, dm2, nl1):
    ...
    pfail2 = float(np.real(1 - np.trace(np.dot(kronecker_product([one, one, one, one, projx, projx, projx]), out2))))
    ...
    pout = float(np.real(1 - np.trace(np.dot(outpostsel2, ideal20to4))))

    # Full-distance computation: determine full distance required for a 100-qubit / 10000-qubit computation
    def logerr1(d):
        return 231 / (pout / 4) * d * float(plog(pphys, d)) - 0.01

    def logerr2(d):
        return 20284 / (pout / 4) * d * float(plog(pphys, d)) - 0.01
